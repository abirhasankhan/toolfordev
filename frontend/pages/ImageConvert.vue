    ...
    <div class="tw-min-h-screen tw-bg-gradient-to-br tw-from-blue-50 tw-to-indigo-100 tw-p-4">
        <div class="tw-max-w-6xl tw-mx-auto">
            ...
            <div class="tw-bg-white tw-rounded-xl tw-shadow-lg tw-p-6 tw-mb-8">
                ...
                <!-- Server Mode Info -->
                <div v-if="processingMode === 'server'"
                    class="tw-mt-4 tw-p-4 tw-bg-blue-50 tw-border tw-border-blue-200 tw-rounded-lg">
                    <p class="tw-text-sm tw-text-blue-800">
                        ℹ️ Server-side processing will attempt to use external APIs and fallback to client-side if
                        unavailable.
                    </p>
                </div>
            </div>
            ...
            <div class="tw-bg-white tw-rounded-xl tw-shadow-lg tw-p-8 tw-mb-8">
                <div @drop="handleDrop" @dragover="handleDragOver" @dragenter="handleDragEnter"
...
const supportedFormats = computed(() => {
    return processingMode.value === 'client'
        ? 'PNG, JPG, GIF, WebP'
        : 'PNG, JPG, GIF, WebP, AVIF, HEIF, TIFF and more'
})
...
// Server-side conversion using a real image processing service
const convertImageServer = async (image) => {
    ...
    try {
        // Using a public image conversion API service
        const response = await fetch('https://api.cloudflare.com/client/v4/images/v1/convert', {
            method: 'POST',
            body: formData,
            headers: {
        ...
        if (!response.ok) {
            ...
            return await convertImageClient(image)
        }
        ...
        console.warn('Server conversion failed:', error.message)
        // Gracefully fallback to client-side conversion
        return await convertImageClient(image)
    }
}
...
const convertImage = async (index) => {
    ...
    try {
        ...
        image.error = processingMode.value === 'server'
            ? 'Server conversion failed and fallback unsuccessful. Please try client-side mode.'
...
const convertAllImages = async () => {
    ...
    if (processingMode.value === 'server') {
        // Server-side batch processing with fallback
        try {
            applyGlobalSettings()
